...
The `home-routes` handler in the `classify.routes.home` namespace
...
The `home-page` function will in turn call the `classify.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/classify/core.cljs` file.



<p class="title is-5">Organizing the routes</p>

The routes are aggregated and wrapped with middleware in the `classify.handler` namespace:
...
Request middleware functions are located under the `classify.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `classify.dev-middleware` namespace found in
